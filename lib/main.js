...
function parseTestsFile(tests_file) {
	...
	// Parse the tests file and remove any comments before parsing
	testsData = JSON.parse(
		("" + fs.readFileSync(tests_file)).replace(/\/\*[\s\S]*?\*\/|\/\/.*?\r?\n/g, '').replace(/^[\w\s]+\(|\);\s*$/g, '')
	);

	function findTests(data) {
		if (data.url) {
			tests.push(data);
		} else if (data.tests) {
			findTests(data.tests);
		} else {
