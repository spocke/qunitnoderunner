...
exports.main = function(argv) {
	...
	var tests = JSON.parse(fs.readFileSync(testFile));
	var conf = config.createConfig(cmdline.get('config'));
	var browsers = browsers = conf.getObj('browsers'), baseUrl;
		...
		if (++testIndex > tests.length - 1 || !currentBrowser) {
			...
			if (browserInfo) {
				...
				currentBrowser = browser.createBrowser({
					...
					url: url.resolve(conf.getStr("baseurl", baseUrl), tests[testIndex].url) + "?reporturl=" + querystring.escape(reportBaseUrl + browserInfo.name + '/' + testIndex),
			...
			return {url : url.resolve(conf.getStr("baseurl", baseUrl), tests[testIndex].url) + "?reporturl=" + querystring.escape(reportBaseUrl + browserInfo.name + '/' + testIndex)};
		}
	};
